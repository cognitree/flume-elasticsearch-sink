...
| es.csv.fields                              | -            | Comma separated csv field name with data type i.e. column1:type1,column2:type2, Supported data types are string, boolean, int and float |
| es.csv.delimiter                           | ,(comma)           | Delimiter for the data in flume event body|
...
````
  ...
  agent.sinks.es_sink.es.csv.fields=id:int,name:string,isemployee:boolean,leaves:float
  agent.sinks.es_sink.es.csv.delimiter=,
````
